...
function transferProjectToRemote(failed, successful) {
  return ssh.putDirectory('../HostMio', '/home/ubuntu/HostMio-temp', {
    recursive: true,
    concurrency: 1,
...
function createRemoteTempFolder() {
  return ssh.execCommand('rm -rf HostMio-temp && mkdir HostMio-temp', {
    cwd: '/home/ubuntu',
  });
}
...
function removeProjectApp() {
  return ssh.execCommand('rm -rf HostMio', {
    cwd: '/home/ubuntu',
  });
}
...
function updateRemoteApp() {
  return ssh.execCommand(
    'mkdir HostMio && cp -r HostMio-temp/* HostMio/ && rm -rf HostMio-temp',
    {
      cwd: '/home/ubuntu',
    }
  );
...
function restartRemoteServices() {
  return ssh.execCommand(
    'cd HostMio && sudo service mongod start && pm2 start bin/www',
    {
      cwd: '/home/ubuntu',
    }
  );
...
function sshConnect() {
  ...
  ssh
    ...
    .then(function () {
      console.log('Removing `HostMio` folder.');
      return removeProjectApp();
    })
    .then(function () {
      console.log('Creating `HostMio-temp` folder.');
